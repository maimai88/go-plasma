...
const (
	clientIdentifier = "plasma" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the plasma command line interface")
...
func init() {
	...
	app.Flags = append(app.Flags, raftFlags...)
...
// plasma is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func plasma(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// LAUNCH PLASMA!!!
	var plasma *plasmachain.PlasmaChain
	if err := stack.Service(&plasma); err != nil {
		log.Info("PLASMA LAUNCH FAILURE")
		utils.Fatalf("Plasma service not running: %v", err)
	}
	log.Info("PLASMA LAUNCH SUCCESS")
