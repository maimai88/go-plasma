...
import (
	...
	"github.com/wolkdb/go-plasma/accounts/keystore"
...
var (
	...
	SQLConfigFlag = cli.StringFlag{
		Name:  "sqlconfig",
		Usage: "path to sql config",
		Value: "/usr/local/sql/etc",
	}
	AppIdFlag = cli.Uint64Flag{
		Name:  "appid",
		Usage: "(D)App identifier (integer, Assigned to each app upon signup)",
		Value: 797,
		//TODO: Value: eth.DefaultConfig.AppId,
	}
	...
	//TODO: Add AppTxPool flags?
	SQLStatsURLFlag = cli.StringFlag{
		Name:  "sqlstats",
		Usage: "Reporting URL of a sqlstats service (nodename:secret@host:port)",
	}
	...
	AppBootnodesFlag = cli.StringFlag{
		Name:  "appbootnodes",
		Usage: "Comma separated enode URLs for (d)app bootstrapping",
		Value: "",
	}
	...
	DepositTokenIdFlag = cli.Uint64Flag{
		Name:  "deposittokenid",
		Usage: "The TokenId associated with the creation of the blockchain",
		Value: uint64(1),
	}

	DepositIndexFlag = cli.Uint64Flag{
		Name:  "depositindex",
		Usage: "The Assigned Deposit Index for a blockchain",
		Value: uint64(0),
	}

	DepositorFlag = cli.StringFlag{
		Name:  "depositor",
		Usage: "The Address of Depositor that created Blockchain",
		Value: "0xA45b77a98E2B840617e2eC6ddfBf71403bdCb683",
	}

	DepositTokenDenominationFlag = cli.Uint64Flag{
		Name:  "deposittokendenomination",
		Usage: "The denomination of the deposit token",
		Value: uint64(1000000000000000000),
	}

	CreatedBlockchainCountFlag = cli.Uint64Flag{
		Name:  "createdblockchaincount",
		Usage: "Number of blockchains created by depositor",
		Value: uint64(1),
	}

	BlockchainIdFlag = cli.IntFlag{
		Name:  "blockchainid",
		Usage: "blockchainid associated with server",
		Value: 77,
	}
	PlasmaAddrFlag = cli.StringFlag{
		Name:  "plasmaaddr",
		Usage: "Address to plasma server",
		Value: "plasma.wolk.com",
	}
	PlasmaPortFlag = cli.IntFlag{
		Name:  "plasmaport",
		Usage: "Port to plasma server",
		Value: 80,
	}
	CloudstoreAddrFlag = cli.StringFlag{
		Name:  "cloudstoreaddr",
		Usage: "Address to cloudstore server",
		Value: "cloudstore.wolk.com",
	}
	CloudstorePortFlag = cli.IntFlag{
		Name:  "cloudstoreport",
		Usage: "Port to cloudstore server",
		Value: 9900,
	}
)
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalIsSet(DataDirFlag.Name):
		...
		log.Debug(fmt.Sprintf("SetNodeConfig  datadir = %s", cfg.DataDir))
	case ctx.GlobalBool(DeveloperFlag.Name):
		cfg.DataDir = "" // unless explicitly requested, use memory databases
	//case ctx.GlobalBool(AppIdFlag.Name):
	//	cfg.AppId = ctx.GlobalUint64(AppIdFlag.Name)
	case ctx.GlobalBool(TestnetFlag.Name):
...
func parsePersistentNodes(path string) []*deep.RaftPeer {
	...
	var nodes []*deep.RaftPeer
	for _, url := range nodelist {
		if url == "" {
			continue
		}
		node, err := deep.ParseComplete(url)
		if err != nil {
